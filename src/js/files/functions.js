...
export function menuInit() {
	if (document.querySelector(".icon-menu, .hamburger")) {
		document.addEventListener("click", function (e) {
			if (bodyLockStatus && e.target.closest('.icon-menu, .hamburger')) {
				bodyLockToggle();
				document.documentElement.classList.toggle("menu-open");
			} else if (e.target.closest('.menu__body a'))  {
				menuClose();
			}
			else if (e.target.closest('.menu__overlay')) {
				menuClose();
			}
		});
	};
...
function windowLoad() {
	...
	if (window.matchMedia) {
		userTheme = window.matchMedia('(preferens-color-scheme: dark)').matches ? 'dark' : 'light';
	}
	window.matchMedia('(preferens-color-scheme: dark)').addEventListener('change', e => {
	...
	if (resetButton) {
		resetButton.addEventListener("ckick", function (e) {
